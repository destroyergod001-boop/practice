...
export class TableApi<DTO extends Record<string, any>, MODEL extends Record<string, any>> {
  ...
  async getById(id: number | string): Promise<MODEL> {
    const res = await apiClient.get<DTO>(`${this.basePath}/id/${id}`);
    return this.parseResponse(res.data) as MODEL;
  }
  ...
  async create(data: Partial<MODEL>): Promise<MODEL> {
    ...
    const res = await apiClient.post<DTO>(`${this.basePath}/create`, payload);
    return this.parseResponse(res.data) as MODEL;
  }
  ...
  async update(conditions: any[], newData: Partial<MODEL>): Promise<number> {
    const payload = {
      conditions,
      newData: this.toDtoPayload(newData as Partial<MODEL>)
    };
    const res = await apiClient.put<{ updatedCount: number }>(`${this.basePath}`, payload);
    return res.data.updatedCount;
  }

  // delete by conditions
  async delete(conditions: any[]): Promise<number> {
    const res = await apiClient.delete<{ deletedCount: number }>(`${this.basePath}`, { data: conditions });
